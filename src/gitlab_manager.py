...
class GitLabManager(RepositoryManager):
    ...
    def get_repositories(self) -> List[Repository]:
        ...
        if self.include_groups:
            self.logger.info(f"🎯 Filtering to specific GitLab groups: {', '.join(self.include_groups)}")
            for group_path in self.include_groups:
                try:
                    ...
                    group_projects = group.projects.list(all=True, get_all=True, include_subgroups=True)
                    self.logger.info(f"📦 Found {len(group_projects)} projects in group {group_path}")
                    ...
                    for project_ref in group_projects:
                        ...
                        try:
                            ...
                            self.logger.debug(f"Could not access project {project_ref.id}: {e}")
                except Exception as e:
        ...
        for project in unique_projects:
            ...
            is_owned_by_user = project.namespace['id'] == self.user.id if is_personal else False
            
            # For corporate repos, we want group/organization projects
            is_corporate = project.namespace['kind'] in ['group', 'project'] or not is_personal
            ...
            if is_corporate or not self.exclude_personal:
                repos.append(Repository(
                    ...
                    is_fork=hasattr(project, 'forked_from_project') and project.forked_from_project is not None,
