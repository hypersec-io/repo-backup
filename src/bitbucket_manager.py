...
class BitbucketManager(RepositoryManager):
    def __init__(self, url: str, token: str, exclude_personal: bool = True, username: str = None):
        ...
        # 1. App Passwords (short tokens, ~36 chars) - use Basic auth with username
        # 2. Workspace Access Tokens (long tokens starting with ATCTT) - use Bearer auth, no username needed
        # Note: ATATT tokens are Atlassian API tokens for Jira/Confluence (NOT Bitbucket)
        self.is_workspace_token = token.startswith('ATCTT') if token else False
        self.is_app_password = not self.is_workspace_token  # If not workspace token, assume app password
        ...
        self.include_groups = [g.strip() for g in include_groups_env.split(',') if g.strip()] if include_groups_env else None
        self.exclude_groups = [g.strip() for g in exclude_groups_env.split(',') if g.strip()] if exclude_groups_env else []
    ...
    def get_repositories(self) -> List[Repository]:
        ...
        if self.is_workspace_token:
            # For workspace tokens, test with repositories endpoint
            test_url = f'https://api.bitbucket.org/2.0/repositories/{self.username}' if self.username else 'https://api.bitbucket.org/2.0/repositories'
        else:
        ...
        if self.is_cloud:
            if self.include_groups:
                # Only get repositories from specified workspaces
                self.logger.info(f"Filtering to specific Bitbucket workspaces: {', '.join(self.include_groups)}")
                for workspace_slug in self.include_groups:
                    try:
                        workspace_repos = self._get_workspace_repos(workspace_slug)
                        self.logger.info(f"Found {len(workspace_repos)} repositories in workspace {workspace_slug}")
                        ...
                        for repo in workspace_repos:
                            ...
                            is_corporate = repo['owner']['type'] in ['team'] or not is_owned_by_user
                            ...
                            if is_corporate or not self.exclude_personal:
                                repos.append(Repository(
                                    ...
                                    platform='bitbucket',
                                    size_kb=repo.get('size', 0) // 1024 if repo.get('size') else None,
                                    default_branch=repo.get('mainbranch', {}).get('name', 'main')
                                ))
                    except Exception as e:
                        self.logger.warning(f"Could not access workspace {workspace_slug}: {e}")
            else:
                ...
                for workspace in workspaces:
                    ...
                    if workspace['slug'] in self.exclude_groups:
                        self.logger.debug(f"Skipping excluded workspace: {workspace['slug']}")
                        continue
    ...
    def _make_request(self, url):
        ...
        if self.is_app_password:
            ...
            if not self.username:
                self.logger.error(f"[ERROR] App passwords require BITBUCKET_USERNAME in .env file")
                return None
        ...
        if response.status_code == 401:
            ...
            if self.is_app_password:
                self.logger.error(f"[ERROR] App password authentication failed. Required permissions:")
                ...
                self.logger.error(f"[ERROR] Create at: https://bitbucket.org/account/settings/app-passwords/")
            else:
                self.logger.error(f"[ERROR] Workspace Access Token authentication failed.")
                self.logger.error(f"[ERROR] To create a Workspace Access Token:")
                self.logger.error(f"[ERROR]   1. Go to your workspace settings")
                self.logger.error(f"[ERROR]   2. Click 'Access tokens' under 'Security'")
                self.logger.error(f"[ERROR]   3. Create token with 'Repositories: Read' permission")
                self.logger.error(f"[ERROR] Note: Workspace tokens are a Premium feature")
            return None
        return response
    ...
    def _get_workspace_repos(self, workspace):
        ...
        while url:
            ...
            if response.status_code != 200:
                self.logger.error(f"Failed to get repos for workspace {workspace}: {response.status_code}")
                self.logger.debug(f"Response: {response.text[:500]}")
                break
        ...
        for link in repo.get('links', {}).get('clone', []):
            if link['name'] == 'https':
                ...
                if self.is_app_password and self.username:
                    ...
                    return url.replace('https://', f'https://x-token-auth:{self.token}@')
                else:
                    # Default fallback
                    return url.replace('https://', f'https://x-token-auth:{self.token}@')
        return None
