...
class LocalBackup:
    def __init__(self, backup_path: str, method: str = 'direct', work_dir: Optional[str] = None):
        """
        ...
        try:
            ...
            self.logger.error(f"[ERROR] Failed to create backup directory {self.backup_path}: {e}")
            raise
    ...
    def _direct_backup(self, repo: Repository, backup_dir: Path, timestamp: str) -> bool:
        ...
        try:
            ...
            if result.returncode != 0:
                self.logger.error(f"[ERROR] Clone failed for {repo.name}: {result.stderr}")
                return False
            ...
            if lfs_check.returncode == 0 and lfs_check.stdout.strip():
                ...
                if lfs_fetch.returncode != 0:
                    self.logger.warning(f"[LFS] LFS fetch failed for {repo.name}: {lfs_fetch.stderr}")
            ...
            if result.returncode != 0:
                ...
                if "Refusing to create empty bundle" in result.stderr:
                    ...
                    self.logger.error(f"[ERROR] Bundle creation failed: {result.stderr}")
                    return False
            
            file_size = bundle_path.stat().st_size
            self.logger.info(f"[SUCCESS] Backed up {repo.name} ({file_size / 1024 / 1024:.2f} MB) to {bundle_path}")
    ...
    def _archive_backup(self, repo: Repository, backup_dir: Path, timestamp: str) -> bool:
        ...
        try:
            ...
            self.logger.info(f"[SUCCESS] Backed up {repo.name} ({file_size / 1024 / 1024:.2f} MB) to {archive_path}")
        ...
        if self.temp_dir.exists():
            ...
            if tmp_parent.name == 'tmp' and tmp_parent.exists():
                try:
                    ...
                    self.logger.info(f"[CLEANUP] Removed empty tmp directory: {tmp_parent}")
                except OSError:
