...
class S3BucketManager:
    ...
    def create_and_configure_bucket(self, bucket_name: Optional[str] = None) -> str:
        ...
        try:
            ...
            if error_code == 'BucketAlreadyOwnedByYou':
                ...
                logger.warning("Bucket creation conflict - trying to proceed with existing bucket")
    ...
    def test_bucket(self, bucket_name: str) -> bool:
        ...
        try:
            ...
            if len(versions.get('Versions', [])) > 1:
                logger.info(f"[PASS] Versioning test passed ({len(versions['Versions'])} versions)")
    ...
    def write_env_file(self, bucket_name: str, env_file: str = '.env', profile_name: str = None):
...
"""
    ...
    def create_iam_user_and_policy(self, bucket_name: str, user_name: str = None) -> tuple:
        ...
        try:
            ...
            if len(existing_keys['AccessKeyMetadata']) >= 2:
                ...
                logger.info("Please delete an existing access key if you need a new one")
    ...
    def create_aws_profile(self, profile_name: str, access_key_id: str, secret_key: str, region: str) -> bool:
...
class RepoBackupOrchestrator:
    def __init__(self, config_path: Optional[str] = None, use_env: bool = True, local_backup_path: Optional[str] = None, work_dir: Optional[str] = None):
        ...
        if use_env:
            ...
            logger.warning("No configuration provided. Use --setup to create bucket and configuration.")
        ...
        if github_token and not github_token.startswith('ghp_your'):
            try:
                manager = GitHubManager(
                    token=github_token,
                    exclude_personal=os.getenv('EXCLUDE_PERSONAL', 'true').lower() == 'true'
                )
        ...
        if bitbucket_token and not bitbucket_token.startswith('your_token_here'):
            try:
                manager = BitbucketManager(
                    ...
                    username=os.getenv('BITBUCKET_USERNAME'),  # Optional for app passwords
                    exclude_personal=os.getenv('EXCLUDE_PERSONAL', 'true').lower() == 'true'
                )
    ...
    def parse_repo_spec(self, repo_spec: str) -> tuple:
        ...
        if '/' not in repo_path:
            raise ValueError(f"Invalid repository format: {repo_spec}. Use owner/repo or platform:owner/repo")
    ...
    def match_repo_pattern(self, pattern: str, repo: Repository) -> bool:
        ...
        if pattern.startswith('re:'):
            ...
            try:
                return bool(re.search(regex_pattern, repo_full, re.IGNORECASE)) or \
                       bool(re.search(regex_pattern, platform_full, re.IGNORECASE))
        ...
        if any(char in pattern for char in ['*', '?', '[']):
            ...
            return fnmatch.fnmatch(repo_full.lower(), pattern.lower()) or \
                   fnmatch.fnmatch(platform_full.lower(), pattern.lower())
        
        # Exact match
        return pattern.lower() == repo_full.lower()
    
    def get_specific_repositories(self, repo_patterns: List[str], platforms: List[str] = None) -> List[Repository]:
        ...
        for pattern in repo_patterns:
            ...
            if pattern_matches:
                ...
                if len(pattern_matches) == 1:
                    ...
                    logger.info(f"[MATCH] Pattern '{pattern}' matched: {repo.platform.upper()}:{repo.owner}/{repo.name}")
                else:
                    logger.info(f"[MATCH] Pattern '{pattern}' matched {len(pattern_matches)} repositories")
                    for repo in pattern_matches[:5]:  # Show first 5
                        logger.debug(f"  - {repo.platform.upper()}:{repo.owner}/{repo.name}")
                    if len(pattern_matches) > 5:
                        logger.debug(f"  ... and {len(pattern_matches) - 5} more")
            else:
                logger.warning(f"[NO_MATCH] Pattern '{pattern}' matched no repositories")
        
        if matched_count != len(specific_repos):
            logger.info(f"[DEDUP] {matched_count} total matches, {len(specific_repos)} unique repositories after deduplication")
    ...
    def get_all_repositories(self, platforms: List[str] = None) -> List[Repository]:
        ...
        if platforms:
            ...
            managers_to_use = [(p, a, m) for p, a, m in self.managers if p in platforms_lower]
            
            if not managers_to_use:
                logger.warning(f"[WARN] No managers found for platforms: {', '.join(platforms)}")
                return all_repos
        
        for platform, account_name, manager in managers_to_use:
            try:
                logger.info(f"[CONNECT] Connecting to {platform.upper()} ({account_name})...")
                repos = manager.get_repositories()
                
                if repos:
                    logger.info(f"[OK] Found {len(repos)} corporate repositories on {platform.upper()} ({account_name})")
                    for repo in repos:
                        logger.debug(f"  - {repo.owner}/{repo.name} ({'private' if repo.is_private else 'public'})")
                    all_repos.extend(repos)
                else:
                    logger.warning(f"[WARN] No repositories found on {platform.upper()} ({account_name})")
                    
            except Exception as e:
                logger.error(f"[ERROR] Failed to fetch repos from {platform.upper()} ({account_name}): {e}")
    ...
    def get_test_repository(self, platforms: List[str] = None) -> List[Repository]:
        ...
        logger.info(f"[TEST] Analyzing {len(all_repos)} repositories to find smallest...")
        ...
        if not valid_repos:
            logger.warning("[TEST] No non-fork, non-empty repositories found for testing")
            return []
        ...
        if test_repo.size_kb:
            ...
            logger.info(f"[TEST] Selected smallest repository: {test_repo.platform.upper()}: {test_repo.owner}/{test_repo.name}{size_info}")
        else:
            logger.info(f"[TEST] Selected repository (size unknown): {test_repo.platform.upper()}: {test_repo.owner}/{test_repo.name}")
    ...
    def backup_repository(self, repo: Repository) -> bool:
        ...
        try:
            logger.info(f"[BACKUP] Backing up {repo.platform.upper()}: {repo.owner}/{repo.name}")
            logger.debug(f"  Repository details: {'Private' if repo.is_private else 'Public'}, "
            ...
            # Report results
            if destinations:
                logger.info(f"[SUCCESS] Successfully backed up {repo.platform.upper()}: {repo.owner}/{repo.name} to {', '.join(destinations)}")
            else:
                logger.error(f"[FAIL] Failed to backup {repo.platform.upper()}: {repo.owner}/{repo.name}")
            
            # Return True if at least one backup succeeded
            return any(results)
            
        except Exception as e:
            logger.error(f"[ERROR] Exception while backing up {repo.platform.upper()}: {repo.owner}/{repo.name} - {e}")
            return False
    
    def run_backup(self, parallel: bool = True, max_workers: int = None, specific_repos: List[str] = None, platforms: List[str] = None, test_mode: bool = False):
        """Run the backup process"""
        if not self.s3_uploader and not self.local_backup:
            logger.error("No backup destination configured. Specify --s3 or --local-path.")
            return
        ...
        if test_mode:
            ...
            logger.info(f"[LIST] Backing up specific repositories: {len(specific_repos)} requested")
            repos = self.get_specific_repositories(specific_repos, platforms)
        else:
            platforms_str = f" from {', '.join(platforms)}" if platforms else ""
            logger.info(f"[DISCOVER] Discovering all accessible repositories{platforms_str}...")
        ...
        logger.info(f"[PROCESS] Using {'parallel' if parallel else 'sequential'} processing "
        ...
        if parallel:
                ...
                with tqdm(total=len(repos), desc="Backing up", unit="repo") as pbar:
                    for future in as_completed(futures):
                        ...
                        try:
                            ...
                            logger.error(f"[ERROR] Error backing up {repo.platform.upper()}: {repo.owner}/{repo.name} - {e}")
                            failed += 1
                        pbar.update(1)
            ...
            with tqdm(repos, desc="Backing up", unit="repo") as pbar:
                for repo in pbar:
                    pbar.set_description(f"[BACKUP] {repo.platform.upper()}: {repo.owner}/{repo.name}")
        ...
        if failed > 0:
            logger.error(f"[WARN] {failed} repositories failed to backup - check logs for details")
            sys.exit(1)
        else:
    ...
    def list_backups(self, platform: str = None):
        ...
        for backup in backups:
            logger.info(f"  - {backup['key']} ({backup['size_mb']:.2f} MB) - {backup['last_modified']}")
...
[bold blue]For more information:[/bold blue] https://github.com/your-org/repo-backup
    ...
    if args.mode in ['local', 'both']:
        ...
        if not local_path:
            logger.error("Local backup requires a path. Provide it as argument or set LOCAL_BACKUP_PATH environment variable.")
            sys.exit(1)
        ...
        if args.profile and not args.list and args.mode == 'local':
            logger.info("[CONFIG] Using current AWS session (profile parameter not needed for local mode)")
    ...
    if args.mode in ['s3', 'both']:
        ...
        if args.setup:
            ...
            if not args.profile and not get_env_default('AWS_PROFILE'):
                logger.error("S3 mode requires --profile or AWS_PROFILE environment variable")
                sys.exit(1)
        
        if args.path and args.mode == 's3':
            logger.info("[INFO] Path not needed for S3-only mode (using bucket configuration)")
    ...
    if args.mode == 's3' and args.setup:
        ...
        if manager.test_bucket(bucket_name):
            ...
            access_key_id, secret_key, user_name = manager.create_iam_user_and_policy(bucket_name)
            
            if access_key_id and secret_key:
                # Create AWS profile for the service
                profile_name = f"repo-backup-{bucket_name}"
                
                if manager.create_aws_profile(profile_name, access_key_id, secret_key, args.region):
                    ...
                    logger.info(f"2. Run: uv run repo-backup s3 --list --profile {profile_name}")
                    logger.info(f"3. Run: uv run repo-backup s3 --profile {profile_name}")
                else:
    ...
    if args.list:
        if args.mode == 'local':
            ...
            if orchestrator.local_backup:
                ...
                if backups:
                    ...
                    for backup in backups:
                        logger.info(f"  - {backup['path']} ({backup['size_mb']:.2f} MB) - {backup['modified']} ({backup['type']})")
                else:
        ...
        if args.all:
            ...
            repo_patterns = [repo.strip() for repo in args.repos.split(',') if repo.strip()]
            logger.info(f"[MODE] Backing up repositories matching {len(repo_patterns)} patterns")
            orchestrator.run_backup(
                parallel=args.parallel,
                max_workers=args.workers,
            ...
            if repo_patterns:
                logger.info(f"[MODE] Backing up repositories matching {len(repo_patterns)} patterns from file")
                orchestrator.run_backup(
                    parallel=args.parallel,
                    max_workers=args.workers,
                    specific_repos=repo_patterns
            ...
            repo_patterns = [repo.strip() for repo in get_env_default('SPECIFIC_REPOS').split(',') if repo.strip()]
            logger.info(f"[MODE] Backing up repositories matching {len(repo_patterns)} patterns from environment")
            orchestrator.run_backup(
                parallel=args.parallel,
                max_workers=args.workers,
                specific_repos=repo_patterns,
            ...
            logger.error("[ERROR] No repositories specified. Use --all to backup all repositories, --test for test mode, or --repos/--repos-file to specify patterns.")
            logger.info("Examples:")
            logger.info("  # Test backup functionality:")
            logger.info(f"  {sys.argv[0]} {args.mode} {'--profile ' + args.profile if args.mode == 's3' and args.profile else ''} --test")
            logger.info("  # Backup all repositories:")
            logger.info(f"  {sys.argv[0]} {args.mode} {'--profile ' + args.profile if args.mode == 's3' and args.profile else ''} --all")
            logger.info("  # Backup specific repository:")
            logger.info(f"  {sys.argv[0]} {args.mode} {'--profile ' + args.profile if args.mode == 's3' and args.profile else ''} --repos 'hypersec-repo/dashboard'")
            logger.info("  # Backup with glob pattern:")
            logger.info(f"  {sys.argv[0]} {args.mode} {'--profile ' + args.profile if args.mode == 's3' and args.profile else ''} --repos 'hypersec-repo/*dashboard*'")
            logger.info("  # Backup with regex pattern:")
            logger.info(f"  {sys.argv[0]} {args.mode} {'--profile ' + args.profile if args.mode == 's3' and args.profile else ''} --repos 're:.*dashboard.*'")
            sys.exit(1)
