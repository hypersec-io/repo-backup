...
class S3Uploader:
    def __init__(self, bucket_name: str, region: str = 'us-east-1', 
        ...
        try:
            ...
            self.logger.error(f"[ERROR] Failed to create working directory {self.temp_dir}: {e}")
            raise
    ...
    def upload_repository(self, repo: Repository, method: str = 'direct') -> bool:
        ...
        try:
            ...
            s3_key = f"{self.prefix}/{repo.platform}/{repo.owner}/{repo.name}_{timestamp}"
    ...
    def _direct_upload(self, repo: Repository, s3_key_base: str) -> bool:
        ...
        try:
            ...
            self.logger.debug(f"Clone command: git clone --mirror [REDACTED_URL] {relative_repo_path}")
            ...
            if lfs_check.returncode == 0 and lfs_check.stdout.strip():
                ...
                if lfs_fetch.returncode != 0:
                    self.logger.warning(f"LFS fetch failed for {repo.name}: {lfs_fetch.stderr}")
                    # Continue anyway - bundle will still work without LFS objects
            
            self.logger.debug(f"Clone command completed - checking repository path exists: {os.path.exists(repo_path)}")
            if not os.path.exists(repo_path):
                self.logger.error(f"Clone appeared successful but repository directory does not exist: {repo_path}")
                self.logger.error(f"Git clone stdout: {result.stdout}")
                self.logger.error(f"Git clone stderr: {result.stderr}")
                return False
            
            if os.path.exists(repo_path):
                self.logger.debug(f"Repository directory contents: {os.listdir(repo_path) if os.path.isdir(repo_path) else 'Not a directory'}")
            ...
            if result.returncode != 0:
                ...
                if "Refusing to create empty bundle" in result.stderr:
                    self.logger.info(f"[SKIP] Skipping {repo.name} - repository is empty (no commits)")
                    # Cleanup
                    if os.path.exists(repo_path):
            ...
            self.logger.info(f"Uploading {repo.name} to S3 ({file_size / 1024 / 1024:.2f} MB)...")
            
            with tqdm(total=file_size, unit='B', unit_scale=True, desc=repo.name) as pbar:
            ...
            self.logger.info(f"Successfully uploaded {repo.name} to s3://{self.bucket_name}/{s3_key}")
